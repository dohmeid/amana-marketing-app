...
const Map: React.FC<MapProps> = ({ data }) => {
    ...
    return (
        <MapContainer center={mapCenter} zoom={6} style={{ height: '100%', width: '100%', backgroundColor: '#1f2937' }}>
            <TileLayer
        ...
        const eventHandlers = {
          ...
          <Circle key={item.name} 
            ...
            radius={circleOptions.radius} eventHandlers={eventHandlers}>
                        <Popup>
                            <div className="font-sans">
                                <strong className="text-lg">{item.name}</strong><br />
                                <span className="text-green-600">Revenue:</span> ${item.revenue.toLocaleString()}<br />
                                <span className="text-red-500">Spend:</span> ${item.spend.toLocaleString()}
                            </div>
                        </Popup>
                    </Circle>
                );
